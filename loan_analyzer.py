...
future_value = 1000
remaining_months = 12
annual_discount_price = .2
def PV (future_value, remaining_months, annual_discount_price):
#def PV(1000, 12, .02):
    PV = future_value / (1+ annual_discount_price / 12) ** remaining_months 
    return PV
...
# YOUR CODE HERE!
PV = PV(future_value, remaining_months, .02)
print(f"{PV}")
...
with open(output_path, 'w',newline='') as csvfile:
        ...
        csvwritr.writerow(header)
        for item in inexpensive_loans:
            csvwritr.writerow(item.values())